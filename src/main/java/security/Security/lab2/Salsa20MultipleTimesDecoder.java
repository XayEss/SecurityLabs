...
//		 char[] cribs = new char[lines];
	...
	public void manual() {
		String[] lines = codedLines.split("\n");
		String[] firstAndOthers = new String[lines.length - 1];
		// for(int i = 0; i < codedLines.length(); i++) {
		System.out.println(Arrays.toString(lines));
		int o = 0;
		for (int j = 1; j < firstAndOthers.length; j++) {
			firstAndOthers[o++] = hexXorLines(lines[0], lines[j]);
			System.out.println(firstAndOthers[o - 1]);
		}
		// }
